...
def db_new_dinner(mp_dinner_title, mp_dinner_user_id, mp_dinner_group_id, mp_dinner_image):
    dinner = Dinner(title=mp_dinner_title, user_id=mp_dinner_user_id,
                    group_id=mp_dinner_group_id, image=mp_dinner_image)
    session.add(dinner)
    session.commit()
    session.close()
...
def db_update_dinner(mp_dinner_id, mp_dinner_title, mp_dinner_user_id, mp_dinner_group_id, mp_dinner_image):
    incoming_dinner = session.query(Dinner).filter(
        Dinner.id == mp_dinner_id).first()
